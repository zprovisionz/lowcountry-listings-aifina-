  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div className="anim-fade-up" style={{ display:'flex', gap:12, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <div style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontSize:'var(--text-ui-label)', color:'var(--text-lo)' }}>
      ...
      <div className="glass-dash anim-fade-up d-100 history-table-wrap" style={{ overflow:'hidden' }}>
        ...
        >
          {['ADDRESS','NEIGHBORHOOD','STATUS','AUTH.','CONF.',''].map(h => (
            <span key={h} style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontSize:'var(--text-ui-label)', color:'var(--text-lo)', letterSpacing:'.14em' }}>{h}</span>
          ))}
        </div>
          ...
          filtered.map((g:Generation, i) => (
            <div key={g.id}>
              <div
                ...
                className="neon-table-row history-table-row"
                style={{
                  display:'grid', gridTemplateColumns:'1fr 130px 90px 80px 80px 76px',
              ...
              >
                <div className="history-row-primary" style={{ overflow:'hidden' }}>
                ...
                <div className="history-cell-desktop" style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontSize:10, color:'var(--text-mid)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', paddingRight:8 }}>
                  {g.neighborhood ?? '—'}
                </div>
                <div className="history-cell-desktop">
                  <span className={`chip chip-${g.status}`}>{g.status}</span>
                </div>
                <div className="history-cell-desktop" style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontWeight:700, fontSize:13, color:scoreColor(g.authenticity_score) }}>
                  {g.authenticity_score!=null ? `${g.authenticity_score}%` : '—'}
                </div>
                <div className="history-cell-desktop" style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontWeight:700, fontSize:13, color:scoreColor(g.confidence_score) }}>
                  {g.confidence_score!=null ? `${g.confidence_score}%` : '—'}
                </div>

                <div className="history-row-meta">
                  <div style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontSize:'var(--text-ui-label)', color:'var(--text-mid)' }}>
                    {g.neighborhood ?? '—'}
                  </div>
                  <span className={`chip chip-${g.status}`}>{g.status}</span>
                  <span style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontWeight:700, fontSize:13, color:scoreColor(g.authenticity_score) }}>
                    {g.authenticity_score!=null ? `Auth ${g.authenticity_score}%` : 'Auth —'}
                  </span>
                  <span style={{ fontFamily:"'DM Mono', ui-monospace, monospace", fontWeight:700, fontSize:13, color:scoreColor(g.confidence_score) }}>
                    {g.confidence_score!=null ? `Conf ${g.confidence_score}%` : 'Conf —'}
                  </span>
                </div>

                <div className="history-row-actions" style={{ display:'flex', gap:6, justifyContent:'flex-end' }}>
                  {g.status==='complete' && (
                    <button
              ...
              {confirmDel === g.id && (
                <div style={{
                  ...
                  <button onClick={()=>handleDelete(g.id)} disabled={deleting===g.id} style={{
                    padding:'8px 16px', minHeight:44, background:'rgba(255,60,60,0.12)',
                    border:'1px solid rgba(255,80,80,0.35)', borderRadius:6,
                    color:'#ff6060', fontFamily:"'DM Mono', ui-monospace, monospace",
                    fontSize:'var(--text-ui-label)', cursor:'pointer',
                  }}>
                    {deleting===g.id ? 'Deleting…' : 'Confirm'}
                  </button>
                  <button onClick={()=>setConfirmDel(null)} style={{ background:'none',border:'none',color:'var(--text-lo)',cursor:'pointer',fontSize:14, minHeight:44, padding:'0 12px' }}>Cancel</button>
                </div>
              )}
            </div>
          ))
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .history-cell-desktop { display: none !important; }
        }
      `}</style>
    </div>
  );
